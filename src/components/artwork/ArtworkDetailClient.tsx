          ...
          {artwork.id === miscUniversityWorkId && (
            <>
              {additionalImagesRaw.length >= 3 && (() => {
                ...
                return (
                  <React.Fragment>
                    <Card key={`misc-carousel-0-${artwork.id}`} className="overflow-hidden rounded-lg">
                      <Carousel opts={{ loop: true }} className="w-full group/carousel">
                        <CarouselContent>
                          {[additionalImagesRaw[0], additionalImagesRaw[1], additionalImagesRaw[2]].map((url, pairIdx) => {
                             if (!url) {return null;}
                             const overallIndex = pairIdx;
                             const imageStyles = getImageTransformAndTransition(url, overallIndex);
                             return (
                               <CarouselItem key={`carousel-0-img-${overallIndex}`}>
                                 <div
                                   ...
                                   className={cn(
                                     ...
                                     zoomedImageIndex === overallIndex ? "z-30 shadow-2xl ring-2 ring-primary" : "z-10 hover:shadow-lg",
                                     zoomedImageIndex === overallIndex && isDragging && !isAnySvg(url) ? "cursor-grabbing" : "",
                                     zoomedImageIndex === overallIndex && !isAnySvg(url) ? "cursor-grab" : "cursor-pointer",
                  ...
                  <Carousel opts={{ loop: true }} className="w-full group/carousel">
                    <CarouselContent>
                      {[additionalImagesRaw[3], additionalImagesRaw[4]].map((url, pairIdx) => {
                        if (!url) {return null;}
                        const overallIndex = (pairIdx === 0 ? 3 : 4);
                        const imageStyles = getImageTransformAndTransition(url, overallIndex);
                        return (
                  ...
                  <Carousel opts={{ loop: true }} className="w-full group/carousel">
                    <CarouselContent>
                      {[additionalImagesRaw[5], additionalImagesRaw[6]].map((url, pairIdx) => {
                        if (!url) {return null;}
                        const overallIndex = (pairIdx === 0 ? 5 : 6);
                        const imageStyles = getImageTransformAndTransition(url, overallIndex);
                        return (
              ...
              {additionalImagesRaw.length >= 9 && (
                 <Card key={`misc-carousel-3-${artwork.id}`} className="overflow-hidden rounded-lg">
                  <Carousel opts={{ loop: true }} className="w-full group/carousel">
                    <CarouselContent>
                      {[additionalImagesRaw[7], additionalImagesRaw[8]].map((url, pairIdx) => {
                        if (!url) {return null;}
                        const overallIndex = (pairIdx === 0 ? 7 : 8);
                        const imageStyles = getImageTransformAndTransition(url, overallIndex);
                        return (
              ...
              {additionalImagesRaw.length > 9 && additionalImagesRaw.slice(9).map((url, idx) => {
                  if (!url) {return null;}
                  const overallIndex = 1 + 8 + (idx + 1);
                  const imageStyles = getImageTransformAndTransition(url, overallIndex);
                  return (
                    <Card
